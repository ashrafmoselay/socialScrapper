...
class Finder():
    ...
    def __find_share(post):
        ...
        try:
            ...
            shares = post.find_element_by_css_selector("[data-testid='UFI2SharesCount/root']").get_attribute(
                'textContent')
    ...
    @staticmethod
    def __extract_comments(item):
        # postComments = item.findAll("div", {"class": "_4eek"})
        postComments = item.find_elements_by_class_name("_4eek")

        # print(postDict)
        comment_list = []
        for comment in postComments:
            comments = dict()
            if comment.find_element_by_class_name("_6qw4") is None:
                continue

            commenterClass = comment.find_element_by_class_name("_6qw4")
            commenter = commenterClass.text
            # print(commenter)
            comments['commenter'] = commenter
            comment_text = comment.find_element_by_class_name("_3l3x").text
            comments["text"] = comment_text
            comments["href"] = commenterClass.get_attribute('href')
            dateContainer = comment.find_element_by_class_name("_6qw9")
            comments["date"] = dateContainer.find_element_by_tag_name("abbr").get_attribute('data-tooltip-content')
            replies = item.find_element_by_class_name('_7a9h')
            if replies:
                comments["reply_count"] = replies.get_attribute('textContent')
                # print(comments["reply_count"])
                # reply = repliesList.find_elements_by_tag_name('li')
                # print("reply here")
                # print(reply)
                # if reply:
                #     print("New comment reply here")
                #     for litag in reply:
                #         print("start loop inside comment")
                #         aria = litag.find_element_by_css_selector("div._4eek")
                #         print(aria)
                #         if aria:
                #             commenter = aria.find_element_by_class_name("_6qw4").text
                #             comments = dict()
                #             comment_text = litag.find_element_by_css_selector("span._3l3x")
                #             if comment_text:
                #                 comments["text"] = comment_text.text
                #                 # print(str(litag)+"\n")
                #
                #             comment_link = litag.find_element_by_class_name("_ns_")
                #             if comment_link is not None:
                #                 comments["link"] = comment_link.get_attribute("href")
                #
                #             comment_pic = litag.find_element_by_class_name("_2txe")
                #             if comment_pic is not None:
                #                 comments["image"] = comment_pic.find_element_by_class_name("img").get_attribute("src")
                #
                #             repliesList = litag.find_element_by_class_name("_2h2j")
                #             if repliesList:
                ...
                #                         if aria2:
                #                             replier = aria2.find_element_by_class_name("_6qw4").text
                #                             if replier:
                #                                 comments['reply'][replier] = dict()
                #
                #                                 reply_text = litag2.find("span", class_="_3l3x")
                #                                 if reply_text:
                #                                     comments['reply'][replier][
                #                                         "reply_text"] = reply_text.text
                #
                #                                 r_link = litag2.find_element_by_class_name("_ns_")
                #                                 if r_link is not None:
                #                                     comments['reply']["link"] = r_link.get_attribute("href")
                #
                #                                 r_pic = litag2.find_element_by_class_name("_2txe")
                #                                 if r_pic is not None:
                #                                     comments['reply']["image"] = r_pic.find_element_by_class_name("img").get_attribute("src")
