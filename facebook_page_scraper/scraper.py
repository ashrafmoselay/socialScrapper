...
class Facebook_scraper:
    ...
    def __init__(self,page_name,posts_count=10,browser="chrome"):
        ...
        # self.URL = "https://www.facebook.com/ADPolytechnic";
    ...
    def scrap_to_json(self):
        ...
        # email_element = self.__driver.find_element_by_id('email')
        # email_element.send_keys('ashrafhassan42@yahoo.com')  # Give keyboard input
        # password_element = self.__driver.find_element_by_id('pass')
        # password_element.send_keys('Ashraf@1010')  # Give password as input too
        # login_button = self.__driver.find_element_by_id('loginbutton')
    ...
    def export_database_to_csv(self,page,keywords,start_at,end_at,requiredPage):
        try:
            ...
            if(page):
                query = query + ' WHERE provider IN (%s)' % ("?," * len(page))[:-1]
            #
            # if(requiredPage):
            #     query = query + ' and name ="'+requiredPage+'"'
            #
            # if(keywords):
            #     k = ""
            #     for q in keywords:
            ...
            #             k = k + " OR "
            #             k = k + ' content like "%' + q + '%"'
            #         # k = k + ' name like "%'+q+'%"' + ' or content like "%'+q+'%"'
            #     query = query + " And ("+k+")"
            #
            # if(start_at):
            #     query = query + " And Date(post_on) >= '"+start_at+"'"
            #
            # if(end_at):
            #     query = query + " And Date(post_on) <= '"+end_at+"'"
            # print(query)
            # query = 'SELECT * FROM posts WHERE id IN (%s)' % ','.join('?' for i in page)

            cur.execute(query, page)
            rows = cur.fetchall()
            # print('here')
            # headers of the CSV file
            fieldnames = ['id', 'name', 'posted_on', 'shares', 'likes', 'loves', 'wow', 'cares', 'sad',
                          'angry', 'haha', 'reactions_count', 'comments', 'content', 'video', 'image',
                          'post_url', 'commenter','comment_text','profile_href','profile_visibility','comment_date',
                          'comment_reply_count'
                          ]
            # open and start writing to CSV files
            filename = os.path.join(ROOT_DIR, 'result.csv')
            with open(filename, 'w', newline='', encoding="utf-8") as data_file:
                ...
                for record in rows:
                    ...
                    com = str(record['post_comments'])
                    com = com.replace("\'", "\"")
                    # allcomments = json.loads(com)
                    allcomments = list(eval(com))
                    # print(type(allcomments))
                    # print(allcomments)
                    # print('here')
                    if allcomments:
                        for commentitem in allcomments:
                            visiblity = 'Private'
                            if commentitem['href']:
                                visiblity = 'Public'
                            row = {
                                    'id': record['post_id'], 'name': record['name'],
                                   'posted_on': record['post_on'], 'shares': record['shares'],
                                   'likes': record['likes'], 'loves': record['loves'],
                                   'wow': record['wow'], 'cares': record['cares'],
                                   'sad': record['sad'], 'angry': record['angry'],
                                   'haha': record['haha'], 'reactions_count': record['reactions_count'],
                                   'comments': record['comments'], 'content': record['content'],
                                   'video': record['video'], 'image': record['image'],
                                   'post_url': record['post_url'],
                                    'commenter': commentitem['commenter'],
                                    'comment_text': commentitem['text'],
                                    'profile_href': commentitem['href'],
                                    'profile_visibility': visiblity,
                                    'comment_date': commentitem['date'],
                                    'comment_reply_count': commentitem['reply_count'],
                                   }
    ...
    def scrap_to_database(self):
        try:
            data = self.scrap_to_json()  # get the data in JSON format from the same class method
            json_data = json.loads(data)
            # iterate over entire dictionary, write each posts as a row to database
            ROOT_DIR = os.path.dirname(__file__)
            databasepath = os.path.join(ROOT_DIR, 'db.sqlite')
            conn = sqlite3.connect(databasepath)
            c = conn.cursor()
            # db.execute("DELETE FROM posts;VACUUM;")
            for key in json_data:
                print('start insert record to database')
                rowdata = [
                    "facebook", key, json_data[key]['name'], json_data[key]['posted_on'],
                    json_data[key]['shares'],
                    json_data[key]['reactions']['likes'], json_data[key]['reactions']['loves'],
                    json_data[key]['reactions']['wow'], json_data[key]['reactions']['cares'],
                    json_data[key]['reactions']['sad'], json_data[key]['reactions']['angry'],
                    json_data[key]['reactions']['haha'], json_data[key]['reaction_count'],
                    str(json_data[key]['comments']), str(json_data[key]['content']), str(json_data[key]['video']),
                    str(json_data[key]['image']), str(json_data[key]['post_url']),
                    str(json_data[key]['post_comments'])
                ]
                # print(rowdata)
                c.execute(
                    "insert into posts (provider, post_id, name, post_on, shares, likes, loves, wow, cares, sad, angry, haha,reactions_count, comments, content, video,image, post_url, post_comments ) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    rowdata)
                conn.commit()
    ...
    def __json_to_csv(self,filename,json_data,directory):
        #headers of the CSV file
        fieldnames = ['id','name','posted_on','shares','likes','loves','wow','cares','sad','angry','haha','reactions_count','comments','content','video'
                    ,'image','post_url','post_comments']
        #open and start writing to CSV files
        with open("{}.csv".format(filename),'w',newline='',encoding="utf-8") as data_file:
            ...
            for key in json_data:
                #parse post in a dictionary and write it as a single row
                row = {'id': key,'name' : json_data[key]['name'],'posted_on': json_data[key]['posted_on'],'shares':json_data[key]['shares'],
                ...
                ,'post_url' : json_data[key]['post_url'],'post_comments' : json_data[key]['post_comments']
                }   
                writer.writerow(row) #write row to CSV file
                # rowdata = [
                #     "facebook", key, json_data[key]['name'], json_data[key]['posted_on'],
                ...
                # # print(rowdata)
                # c.execute(
                #     "insert into posts (provider, post_id, name, post_on, shares, likes, loves, wow, cares, sad, angry, haha,reactions_count, comments, content, video,image, post_url ) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                #     rowdata)
                # conn.commit()
                # print('after insert record')
            data_file.close() #after writing close the file
    ...
    def __find_elements(self):
        ...
        for post in all_posts:
            try:
                ...
                #find comments from the post
                post_comments = Finder._Finder__extract_comments(post)
                ...
                # com = str(post_comments)
                # com = com.replace("\'", "\"")
                # allcomments = json.loads(com)
                # for key in allcomments:
                #     print(key, " :", commentitem)
